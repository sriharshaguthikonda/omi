...
- **RESEARCH OVERRIDE (docs/research/R3-android-media-button-reality.md — a prior ChatGPT deep-research run, authoritative):**
  - AVRCP media buttons are **best-effort + single-consumer**: our `MediaSessionCompat` only wins while no real music app owns the media-button session. UX copy must say so (increment-1 already does). Don't promise "any button any device".
  - `KeyEvent.getDeviceId()` is **unreliable for AVRCP** → default media-key mappings to **AMBIGUOUS** tier; keep the attribution chain but don't block on it. Reliable per-device attribution only comes from BLE GATT (Task 5).
  - **Press-pattern (single/double/long) detection on media keys is unreliable** — the framework/device collapses double-tap into NEXT. So the primary "select which input" is **which discrete media keycode the device delivers** (PLAY_PAUSE / NEXT / PREVIOUS / STOP), captured in learn mode. Press-pattern is **best-effort only / de-scoped from MVP** — do NOT build an elaborate `PressPatternDetector` as the core. Extend `BtMediaButtonTrigger` to capture ALL media keycodes and route each through the mapping engine.
- **Ship order for THIS pass (Claude scope):** Tasks 1→4 reframed by R3 — Task 1 registry, Task 2 = **extend `BtMediaButtonTrigger` to capture all discrete media keycodes + AMBIGUOUS-default attribution** (not a press-pattern engine), Task 3 mapping engine, Task 4 learn wizard (pick the keycode the device delivered → assign action). Tasks 5 (GATT, the only truly per-device tier), 6 (active-device), 7 (matrix doc) are the fast-follow, not this pass.
