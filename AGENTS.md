...
- Android dev APK CI: any push touching `app/**` builds a dev-flavor APK (`.github/workflows/android_apk_build.yml`); pushes to `main` also refresh the rolling `apk-latest` prerelease. Bootstrap mirrors `app/setup.sh` android (prebuilt Firebase configs + shared debug keystore + web-auth `.dev.env` before build_runner); `CM_KEYSTORE_*` env vars neutralize the Codemagic `CI=true` branch in `app/android/app/build.gradle`.
